-- ============================================
-- SEED: review_answers_in_progress
-- Mirrors all rows from review_answers_submitted
-- has_unpublished_changes = false
-- submitted_review_answers_id points to submitted row
-- reviewed_by resolved via profiles <- auth.users (email)
-- ============================================
...
-- =========================================================
-- URL CASES (10) -> 20 in_progress rows
-- =========================================================

-- 1) Case: MDR (1111...)
(
  'a1000001-7b85-4b98-9e2a-1f98b4a9a1c1',
  '11111111-1111-4111-8111-111111111111',
  (SELECT p.id FROM public.profiles p JOIN auth.users u ON u.id = p.id WHERE u.email = 'gorm-labenz@hotmail.com'),
  '{
...
(
  'a1000002-3d90-4e8d-bc70-6b8b5a2d8c91',
  '11111111-1111-4111-8111-111111111111',
  (SELECT p.id FROM public.profiles p JOIN auth.users u ON u.id = p.id WHERE u.email = 'anna.schmidt@example.com'),
  '{
...
(
  'a1000003-3b8c-4d37-8bb7-7b4c0d3f2e11',
...
(
  'a1000004-9fb1-4d02-a1b3-8b8f5c03f9f3',
  '0de6a28d-db36-4c73-9131-e8ccbce69557',
  (SELECT p.id FROM public.profiles p JOIN auth.users u ON u.id = p.id WHERE u.email = 'anna.schmidt@example.com'),
  '{
...
(
  'a1000005-6b8a-4c2e-9fe7-1e4b8c1cc9a1',
...
(
  'a1000006-cf7f-4b2f-a22a-2a9a2b58c12f',
  'f2974bce-eeca-41d0-99d2-8c73ed966dcf',
  (SELECT p.id FROM public.profiles p JOIN auth.users u ON u.id = p.id WHERE u.email = 'anna.schmidt@example.com'),
  '{
...
(
  'a1000007-bbd8-4df6-8a9f-bfd5d42cba7c',
...
(
  'a1000008-93f6-4d38-a6f5-6d1c1b58bff1',
  '9c2b5f2f-7701-4d79-ac44-458dba098775',
  (SELECT p.id FROM public.profiles p JOIN auth.users u ON u.id = p.id WHERE u.email = 'anna.schmidt@example.com'),
  '{
...
(
  'a1000009-1d87-46ac-a1f4-b0e3d1c3bb11',
...
(
  'a1000010-63b6-4e65-9b1e-ea0e4d6b9a44',
  'd3c3f904-470b-4fdc-b978-df66ac90043a',
  (SELECT p.id FROM public.profiles p JOIN auth.users u ON u.id = p.id WHERE u.email = 'anna.schmidt@example.com'),
  '{
...
(
  'a1000011-0c2b-49f1-9b25-7f1f8f5e8c19',
...
(
  'a1000012-6db4-4b46-9f8d-2a786a0f43fe',
  '6b6f0f45-ef48-4f50-931f-46ad0cd3c3b5',
  (SELECT p.id FROM public.profiles p JOIN auth.users u ON u.id = p.id WHERE u.email = 'anna.schmidt@example.com'),
  '{
...
-- 7) Case: AP (c8c0f221...)
(
  'a1000013-7b2d-44aa-9f90-7b2e9a4c1b11',
...
(
  'a1000014-9a7c-4db7-a7a9-8c1a7d9c8b11',
  'c8c0f221-5c91-4c2f-820b-66b7d84bb7d2',
  (SELECT p.id FROM public.profiles p JOIN auth.users u ON u.id = p.id WHERE u.email = 'anna.schmidt@example.com'),
  '{
...
(
  'a1000015-5b63-4681-9d1b-5f1c3a8d4e21',
...
(
  'a1000016-4d8e-4b79-9c32-2f8a0b3e4d21',
  '3a7c2a67-0f41-4c17-a5f8-2e6b4ad0a9d7',
  (SELECT p.id FROM public.profiles p JOIN auth.users u ON u.id = p.id WHERE u.email = 'anna.schmidt@example.com'),
  '{
...
(
  'a1000017-9d3a-4c2f-9c91-9e7c4b2a1c10',
...
(
  'a1000018-8b31-4a73-9f88-9a35a2b1c3d4',
  '9f4b66d3-70f3-4d8c-9d35-3f3b64e3c46b',
  (SELECT p.id FROM public.profiles p JOIN auth.users u ON u.id = p.id WHERE u.email = 'anna.schmidt@example.com'),
  '{
...
(
  'a1000019-5c3c-4a23-9b8a-0e1c2d3f4a55',
...
(
  'a1000020-0d11-49b8-9a01-1c2b3a4d5e66',
  '0a3c6f1a-9360-4a27-8bf2-58b5b4f4983f',
  (SELECT p.id FROM public.profiles p JOIN auth.users u ON u.id = p.id WHERE u.email = 'anna.schmidt@example.com'),
  '{
...
-- =========================================================
-- TEXT CASES (10) -> 20 in_progress rows
-- =========================================================

-- 1) Case: 2222 (Klima-Text)
(
  'b2000001-7b2b-4a2a-9b2b-8c2a1b0c2a12',
...
(
  'b2000002-7b2b-4a2a-9b2b-8c2a1b0c2a13',
  '22222222-2222-4222-8222-222222222222',
  (SELECT p.id FROM public.profiles p JOIN auth.users u ON u.id = p.id WHERE u.email = 'anna.schmidt@example.com'),
  '{
...
(
  'b2000003-1a2b-4c3d-9e1f-0a1b2c3d4e51',
...
(
  'b2000004-1a2b-4c3d-9e1f-0a1b2c3d4e52',
  'c3a9ef3c-3c4a-4ff2-b5a3-f7d31a8bb2b0',
  (SELECT p.id FROM public.profiles p JOIN auth.users u ON u.id = p.id WHERE u.email = 'anna.schmidt@example.com'),
  '{
...
(
  'b2000005-2e3f-4a5b-9c6d-1e2f3a4b5c61',
...
(
  'b2000006-2e3f-4a5b-9c6d-1e2f3a4b5c62',
  '8d1a4a35-8b7c-44c1-8f1a-1a4d1e8d3b5e',
  (SELECT p.id FROM public.profiles p JOIN auth.users u ON u.id = p.id WHERE u.email = 'anna.schmidt@example.com'),
  '{
...
(
  'b2000007-1111-4a2b-8c3d-4e5f6a7b8c71',
...
(
  'b2000008-1111-4a2b-8c3d-4e5f6a7b8c72',
  '2b0c9d51-4a1f-4c2e-9a4f-5e2b1e7c0a9f',
  (SELECT p.id FROM public.profiles p JOIN auth.users u ON u.id = p.id WHERE u.email = 'anna.schmidt@example.com'),
  '{
...
(
  'b2000009-3e4f-4a5b-9c6d-7e8f9a0b1c81',
...
(
  'b2000010-4f50-4a5b-9c6d-7e8f9a0b1c82',
  'bd1f0b74-1c77-4e1b-9a1f-0b7c1a1d2e3f',
  (SELECT p.id FROM public.profiles p JOIN auth.users u ON u.id = p.id WHERE u.email = 'anna.schmidt@example.com'),
  '{
...
(
  'b2000011-5061-4a2b-8c3d-4e5f6a7b8c91',
...
(
  'b2000012-6171-4a2b-8c3d-4e5f6a7b8c92',
  '7c0b1d2e-3f4a-5b6c-7d8e-9f0a1b2c3d4e',
  (SELECT p.id FROM public.profiles p JOIN auth.users u ON u.id = p.id WHERE u.email = 'anna.schmidt@example.com'),
  '{
...
(
  'b2000013-7181-4a2b-8c3d-4e5f6a7b8ca1',
...
(
  'b2000014-8191-4a2b-8c3d-4e5f6a7b8ca2',
  '9a8b7c6d-5e4f-3a2b-1c0d-9e8f7a6b5c4d',
  (SELECT p.id FROM public.profiles p JOIN auth.users u ON u.id = p.id WHERE u.email = 'anna.schmidt@example.com'),
  '{
...
(
  'b2000015-9201-4a2b-8c3d-4e5f6a7b8cb1',
...
(
  'b2000016-0301-4a2b-8c3d-4e5f6a7b8cb2',
  '4d3c2b1a-0f9e-8d7c-6b5a-4f3e2d1c0b9a',
  (SELECT p.id FROM public.profiles p JOIN auth.users u ON u.id = p.id WHERE u.email = 'anna.schmidt@example.com'),
  '{
...
(
  'b2000017-0405-4a2b-8c3d-4e5f6a7b8cc1',
...
(
  'b2000018-0506-4a2b-8c3d-4e5f6a7b8cc2',
  '1a2b3c4d-5e6f-7081-92a3-b4c5d6e7f809',
  (SELECT p.id FROM public.profiles p JOIN auth.users u ON u.id = p.id WHERE u.email = 'anna.schmidt@example.com'),
  '{
...
(
  'b2000019-0607-4a2b-8c3d-4e5f6a7b8cd1',
...
(
  'b2000020-0708-4a2b-8c3d-4e5f6a7b8cd2',
  '89abcdef-0123-489a-8bcd-ef0123456789',
  (SELECT p.id FROM public.profiles p JOIN auth.users u ON u.id = p.id WHERE u.email = 'anna.schmidt@example.com'),
  '{
