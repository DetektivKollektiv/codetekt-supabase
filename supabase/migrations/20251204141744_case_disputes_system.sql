-- ============================================
-- MIGRATION: Case Disputes System
-- Table: case_disputes
-- ============================================
...
create table "public"."case_disputes" (
...
-- Primary Key
create unique index case_disputes_pkey on public.case_disputes using btree (id);
alter table "public"."case_disputes" add constraint "case_disputes_pkey" primary key using index "case_disputes_pkey";

-- Nur ein offener Dispute pro Feld pro Case (kritisch!)
create unique index idx_case_disputes_one_open 
  on public.case_disputes(case_id, field_id) 
  where resolved_at is null;

-- Admin-Queue Index (für schnelle Abfragen offener Disputes)
create index idx_case_disputes_pending 
  on public.case_disputes(created_at) 
  where resolved_at is null;

-- Lookup-Indizes
create index case_disputes_case_id_idx on public.case_disputes using btree (case_id);
create index case_disputes_disputed_by_idx on public.case_disputes using btree (disputed_by);
create index case_disputes_resolved_by_idx on public.case_disputes using btree (resolved_by);
...
grant select, insert on table "public"."case_disputes" to "authenticated";
grant all on table "public"."case_disputes" to "service_role";
